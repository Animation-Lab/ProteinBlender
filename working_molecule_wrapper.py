...
class MoleculeWrapper:
    ...
    def _setup_domain_network(self, domain: DomainDefinition, chain_id_int_str: str, start: int, end: int) -> bool:
        ...
        if not (residue_attr_name in attrs and chain_attr_name in attrs and position_attr_name in attrs):
            print(f"ERROR MoleWrap._setup_domain_network: Missing required attributes ('{residue_attr_name}', '{chain_attr_name}', or '{position_attr_name}') on parent molecule mesh. Cannot find C╬▒. Domain: {domain.id}")
            return False

        ca_pos_start = None
        print(f"DEBUG MoleWrap._setup_domain_network: Searching for C╬▒ for START of domain {domain.name}, target chain_id_int_str: '{chain_id_int_str}', target res_id: {start}")
        for i in range(len(attrs[residue_attr_name].data)):
            ...
            if atom_chain_id_val == chain_id_int_str and atom_res_num == start and is_ca:
                ...
                print(f"DEBUG MoleWrap._setup_domain_network: Found C╬▒ for START of domain {domain.name} at atom index {i}. Position: {ca_pos_start}")
                break
        ...
        if ca_pos_start is None:
            ...
            for i in range(len(attrs[residue_attr_name].data)):
                if str(attrs[chain_attr_name].data[i].value) == chain_id_int_str and attrs[residue_attr_name].data[i].value == start and count < 5:
                    ...
                    count += 1
            return False # Cannot proceed without C╬▒ for start

        # Set the domain object's location to the C╬▒ of the start residue (in world space)
