...
class MOLECULE_PB_OT_create_domain(Operator):
    ...
    def set_pivot_to_alpha_carbon(self, context, molecule, domain):
        ...
        This function handles both textual atom names (CA, N, C) and numeric atom identifiers (1, 2, 3),
        as well as both alphabetic (A, B, C) and numeric (0, 1, 2) chain IDs.
        """
        try:
            ...
            required_attrs = ["atom_name", "chain_id", residue_attr]
            ...
            # Convert between alphabetic and numeric chain IDs
            search_chain_ids = self._get_possible_chain_ids(domain_chain_id)
            ...
            # Check if we have numeric or textual atom names
            numeric_atom_names = self._check_numeric_atom_names(atom_names)
            
            # Find the alpha carbon position
            ...
            if "position" in attrs:
                ...
                if numeric_atom_names:
                    # Handle numeric atom identifiers
                    alpha_carbon_pos = self._find_alpha_carbon_numeric(
                        positions, atom_names, chain_ids, res_nums, 
                        search_chain_ids, start_res
                    )
                else:
                    # Handle textual atom names (search for "CA")
                    alpha_carbon_pos = self._find_alpha_carbon_textual(
                        positions, atom_names, chain_ids, res_nums, 
                        search_chain_ids, start_res
                    )
            ...
            print(f"Error setting pivot to alpha carbon: {str(e)}")
    
    def _get_possible_chain_ids(self, chain_id):
        """Convert between alphabetic and numeric chain IDs."""
        search_chain_ids = [chain_id]
        
        if chain_id.isalpha():
            # If chain ID is alphabetic (A, B, C), also try numeric index (0, 1, 2)
            try:
                # A = 0, B = 1, etc.
            ...
            # If chain ID is numeric (0, 1, 2), also try alphabetic version (A, B, C)
            try:
                # 0 = A, 1 = B, etc.
                alpha_chain = chr(int(chain_id) + ord('A'))
                search_chain_ids.append(alpha_chain)
            except:
                pass
                
        return search_chain_ids
    
    def _check_numeric_atom_names(self, atom_names):
        """Determine if we have numeric atom names."""
    ...
    def _find_alpha_carbon_numeric(self, positions, atom_names, chain_ids, res_nums, 
                                  search_chain_ids, start_res):
        """Find the alpha carbon position for numeric atom identifiers."""
        ...
        if start_res in residue_atoms and len(residue_atoms[start_res]) >= 2:
            # In PDB format with numeric atom types, the alpha carbon is typically 
            # the 2nd atom (index 1) of each amino acid
            alpha_carbon_idx = residue_atoms[start_res][1]
            return positions[alpha_carbon_idx].vector
            
        return None
    
    def _find_alpha_carbon_textual(self, positions, atom_names, chain_ids, res_nums, 
                                  search_chain_ids, start_res):
        """Find the alpha carbon position for textual atom names."""
        for i in range(len(atom_names)):
