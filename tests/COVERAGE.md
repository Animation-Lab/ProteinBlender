...
| `test_saveload.py` | the cases: 20 builders x round trip, 3 x second generation, 2 x render |
...
| `test_live_assembly_ui.py` | native layouts expose one Create New Assembly workflow and child pencil for deposited, cyclic, and helical assemblies; Apply/Cancel/OK, evaluated copy counts, shared child controls with helical-only bend options, and deletion undo/redo in normal installed profiles |
| `test_live_lighting.py` | Set Up Lighting beside the keyframe tools, actual preset popup, cancellation, OK, viewport scene-light preview, repeated application and undo/redo in installed Blender profiles |
| `test_live_outliner_palette.py` | actual PB Outliner screenshot pixels show RGBY bands on both protein and chain rows and compare their width and height with the solid domain swatches; real mouse clicks on either half open the shared-color wheel and recolor every domain, with undo/redo restoring mixed/solid indicators |
...
| `test_lighting.py` | evaluated 5IM3 assembly/point-cloud bounds, DNA and membrane fitting, scale-correct light power, hidden geometry, camera alignment, preservation/restoration of existing lighting, and rendered RGB/shape checks for all three presets; the lighting roundtrip also preserves light settings, world nodes and original-light references |
| `test_conformations.py` | sequence-guided chain matching, independent 1AKE/4AKE superposition, rigid-transform/pivot invariance, insertion codes, gaps and substitutions, minimum stable-core size, animated geometry and rendered cartoon/surface endpoints/midpoint and recoloring, timing/style controls, visibility ownership and deletion; cartoon/surface conformational roundtrips preserve representation, atom identity, shape coordinates and their F-curves |
...
| `test_symmetry_bend.py` | bending a generated helical filament by re-placing its subunits (`core/symmetry_bend.py`): a straight bend reproducing the analytic helix written out in the test, subunits landing at *arc length* along the path (with a count chosen so an even-fractions implementation parts company from an arc-length one), a filament longer than its curve carrying straight on rather than piling up, each subunit standing up on the local tangent, the twist still accumulating, and every operator being a rigid motion (`R R^T = I`, `det = +1`) - the invariant that separates moving a subunit from deforming one. Then the rig through its operators: the curve spanning the filament, adding a bend moving nothing until a node is dragged, a drag re-placing the copies live *with no build call in between* (measured on the depsgraph), the middle of the filament following the middle handle while the anchored original stays put, removal restoring the straight filament exactly and taking its objects with it, the node count changing without losing the bend, presets shaping the path, and Realize landing each copy on the instance it replaced |
| `test_symmetry_dialog.py` | shared assembly dialog: generated symmetry and deposited BMT create children of their protein/chain/domain source; evaluated copy counts; Apply previews, OK commits, Cancel restores the previous state, including previews on multiple proteins; edit settings persist with the build; deposited picker excludes identity-only assemblies |
...
| `test_assembly_build.py` | building *and animating* a deposited assembly inside ProteinBlender: which assemblies a structure offers, identity-only ones being withheld, assembly controls opening only through a clicked child, for both assembly sources, and targeting that child even after another protein is imported, one instance placed per `REMARK 350` operator *on the domain objects* (measured from the depsgraph and from rendered pixels, not the node graph), clear restoring the asymmetric unit exactly, rebuilds not stacking, the build/clear/keyframe operators, and the assemble/disassemble animation: factor 0 returning every copy exactly onto the asymmetric unit, factor 1 reproducing the deposited placement, intermediate factors matching the file's own operator rotated part-way about its own axis, per-copy monotonic travel, stagger putting copies at different stages mid-animation, the factor surviving as keyframes, coexistence with Brownian jitter on a puppet over the same protein, and deleting the protein taking its assembly datablocks with it. Also the asymmetric unit as a first-class picker entry: leading the list without displacing any real assembly, being what an untouched picker opens on, taking a deposited assembly *or* a generated symmetry back off (measured on the depsgraph, not the tag), being harmless when asked for twice, and the picker following what is actually built |
...
| `test_outliner_colors.py` | the PB Outliner's row colour swatches: seeding from what each item renders with (node graph as ground truth), actual-color palettes for mixed proteins/chains with up to four visible bands, domain-count/overflow tooltips, rebuilding and returning to a solid swatch (including true grey), preview cleanup, and a pick on a protein/chain/domain row recolouring exactly what that row covers |

## Behaviour regressions (guard against reintroduction)

- **Lighting fits evaluated molecular geometry:** GN iteration reuses temporary object pointers and point-cloud object boxes can be stale. Lighting caches by evaluated data identity and reads atom positions/radii. The assembly fit and real render tests caught the original undersized/empty fit before correction; native popup tests cover cancellation and undo/redo, and the save/load lane verifies the persistent rig and original-light references. Workbench switching has a red/green regression for Blender 5's Eevee identifier; lit-versus-unlit renders verify actual light contribution in both Eevee and Cycles.

- **Mixed swatches were much smaller than solid swatches.** The live screenshot regression measured 14px versus 35px before the fix. Two adjoining preview tiles fill the normal swatch width while retaining the shared picker and tooltip; the test compares visible pixels rather than layout allocation. Integration coverage checks both tile buffers and their cleanup.

- **Mixed-color outliner indicators:** `test_outliner_colors.py` replaces the old mixed-grey expectation with actual domain palettes and verifies the four-band icon buffer and shared-picker targeting. The foreground UI lane checks opening/dismissing the picker preserves colors; `test_live_outliner_palette.py` observes the visible bands and clicks the color wheel to recolor the whole protein, then verifies undo and redo. A plain scene-bound `invoke_popup` wheel omitted an undo step; the regression failed before switching to Blender's automatic operator popup.

- **Membrane-owned force fields:** `test_membrane_targets.py` checks independent membranes, chain/domain targeting, repartitioning, and rename persistence; the force-fields roundtrip builder saves two membranes with different targets and spacing. `test_live_membrane_targets.py` verifies the resulting lipid clearance in normal Blender sessions.
- **One assembly workflow:** deposited assemblies now use Create New Assembly and the child edit pencil, sharing Apply/OK/Cancel with generated symmetry. `test_assembly_build.py` reproduced the duplicate panel and wrong-protein animation target before the fix; it now checks child access and slider isolation. A Blender 5.1 regression also verifies building the second loaded protein: a hidden string ID now separates public operator arguments from the dynamic UI enum. Assembly deletion removes its child immediately and rebuilding preserves the clicked row. The old bend test only asserted a scene enum and was removed; foreground coverage observes actual draw calls for helical versus cyclic controls. `test_live_assembly_ui.py` inspects actual native layouts for Create New Assembly and the child pencil, exercises Apply/Cancel/OK, checks helical-only bend controls, and verifies child deletion through undo/redo in normal Blender profiles.
- **Symmetry beneath its source:** `test_symmetry_dialog.py` checks protein/chain/domain parents and evaluated copy counts; `scoped_symmetry` exercises save/load.
- **5IM3 biological matrices:** `test_biological_assembly.py` builds assembly 1 through the BMT dialog from PDB and mmCIF, then checks copy separation against independent ATOM coordinates and BIOMT matrices.
- **Selected split-domain opacity:** the foreground UI suite selects a row without dragging a boundary, checks its opaque material and residue range, then splits it and checks the preview follows immediately. Cancellation restores the original materials. `test_live_domain_focus.py` also measures actual viewport opacity on deployed Blender profiles.
...
### Structural alignment and playback (2026-09-07)

- Native popup regression: End changed geometry but left the scrubber at 50%; the live test failed with that exact value before the slider was made a view of the scene frame.
- Stable-core regression: highly divergent 214-residue coordinates retained only 99 fitting pairs; the test failed before pruning was bounded at half the pairs (107).
- Imported atom identity follows the solvent-filtered mesh rather than the unfiltered MolecularNodes array. Exact residue/atom identifiers persist for subsequent alignment after file load.
- Transition-specific tests and save/reopen pass in Blender 5.0 and 5.2; deployed native popup creation/cancel, scrubbing/playback, Done, undo/redo pass in Blender 5.1 and 5.2. The final Blender 5.0 focused run passes 12 checks, including the UI-timer persistence contract and rendered color changes. Final full suites: **719 passed, 7 skipped, 1 existing xfail** on both Blender 5.2 (602.18 s) and 5.1 (599.47 s), exit 0. The strengthened displayed-geometry pivot regression also passes separately on 5.2.
- Rendered color regression: the intended blue transition rendered pink because its fresh node tree lacked Set Color; a pixel test failed before enabling that node path. Playback color changes are also verified by a red render.
- Normal-profile save/reopen: a 5.2-created transition file plays in the deployed 5.1 profile, and its saved structures create another 214-residue alignment without re-import. All six installed addon copies are byte-verified.
- Validation limitation: the final 5.2/5.1 full runs emitted 6/2 Windows native access-violation diagnostics while continuing to completion. These resemble diagnostics recorded in earlier feature work; their cause is not established. No Python test failed, and the focused 5.0, deployed popup, and normal-profile reopen checks were clean. Logs: `/tmp/pb-conformations-final-full52.log`, `/tmp/pb-conformations-final-full51.log`.

### Surface in conformational playback (2026-09-07)

- The playback popup omitted Surface although the shared style system supports it. Before adding the enum entry, the new rendered-surface test failed with `enum "surface" not found` at the public edit operator.
- The render regression now covers both Cartoon and Surface at the start, midpoint and end of the 1AKE/4AKE chain A transition, asserts changing silhouettes and blue color, and verifies recoloring to red. A separate Surface save/reopen case preserves the representation and animation in a fresh Blender process.
- Focused Surface render and persistence checks pass in Blender 5.0 and 5.2 (2 each). Installed native popup tests pass in normal-profile 5.1 and 5.2 (2 each), including Surface selection, Done, reopening, scrubbing and playback. The test's RNA lookup uses the registered operator type rather than the base Operator RNA.
- Visually reviewed three 512-pixel Cycles renders and the native Surface popup from deployed Blender 5.2. All six installed addon copies are byte-verified.
- Full suites: **721 passed, 7 skipped, 1 existing xfail** on both Blender 5.2 (597.99 s) and 5.1 (587.92 s), exit 0. The 5.2 run emitted no native access-violation diagnostics; 5.1 emitted 10 while continuing to completion. Their cause remains undetermined. Logs: `/tmp/pb-conformation-surface-full52.log`, `/tmp/pb-conformation-surface-full51.log`.

### Continuous cartoon motion (2026-09-07)

- Reproduced a **4.72 Å** arrow jump in one frame of 1AKE → 4AKE chain A despite atom steps of about 0.36 Å. The final regression fails with that same signature when the stabilization calls are removed, then passes with the fix restored.
- Both sheet and helix rendering contain thresholded orientation corrections. The transition records their choices on the starting conformation and reuses them while the actual normals and atom positions animate. Ordinary protein cartoon assets are left intact; the transition uses copies of the affected groups. No playback handler or driver is needed for the fix.
- The five continuity cases cover new transitions, upgrading an old transition at frame 50, reopening without the original cartoon template, switching through Spheres/Surface/Cartoon, and whole-protein playback. They check every frame, arbitrary-order scrubbing, no leaked shape keys, and the starting surface against the untouched MN asset. The surface comparison uses bidirectional nearest-point distances at the original 1e-6 tolerance because a 180° rectangular-profile correction can permute equivalent vertex indices.
- An initial temporary mesh copy left an invalid orphan Key datablock in Blender 5.0, causing the following import test to fail. Copying geometry and attributes through BMesh avoids creating that temporary Key.
- Save/reopen snapshots include both sheet and helix reference-turn values and their version. Native popup coverage includes single-chain and whole-protein upgrades, continuous scrubbing and playback, plus the existing cancellation, Done, undo/redo and Surface workflows.
- A normal-profile 5.1 reopen exposed a missing-template upgrade failure: Blender had discarded the unused original `Style Cartoon` group. The automated reproduction failed with that exact KeyError before upgrading was changed to restore the bundled template when needed.
