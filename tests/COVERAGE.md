...
- **Dragging the FIRST domain's Start stalled one residue in and left a 1-1 domain behind.**
  ...
  Fixed by not restructuring *while the value is moving*. The row is added by a debounced app timer (`_queue_layout` / `_settle`, 0.25s) once the boundary stops changing, so the drag runs to wherever the user takes it and the new range adjuster appears when they let go - editable like any other row, with its neighbour wired to it. `execute` still completes the layout through the new `domain_layout.complete_layout`, so committing before the timer fires cannot lose those residues either, and the dialog says "Residues 1-29 will become a new domain" for as long as the gap is open.
  The timer is registered in `not_load_related` in `test_persistence_contract.py`: it is scheduled from a dialog property edit, never a load handler, and touches only the dialog's own rows.
  Guarded by `test_domain_splitter.py::test_complete_layout_gives_an_orphaned_head_its_own_domain`, `::test_complete_layout_gives_an_orphaned_tail_its_own_domain`, `::test_complete_layout_leaves_a_layout_that_already_tiles_alone` and `::test_complete_layout_names_what_it_creates` for the arithmetic, plus `tests/ui/run_ui_scenarios.py::edit_chain_domains_first_start_drag` (no row appears *during* the drag, and the value reaches its target) and `::assert_first_start_settled_into_a_row` (the row is there afterwards, covers the orphaned stretch, and drives its neighbour like any other boundary) - the only lane that can reach any of this, since the headless lane never calls the row-element update path at all. That scenario commits through the live instance's `execute()` rather than `layout_json`, which is taken literally and would prove nothing about the completion; it runs on the **last** chain because it is the only committing scenario and the undo scenarios below assume the first chain's domain ordering.
