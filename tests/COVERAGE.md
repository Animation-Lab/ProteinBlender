...
| `test_symmetry_dialog.py` | the Symmetry Builder dialog's three ways out, counted off the depsgraph rather than off the assembly tag: OK building what was on screen and putting a row under its protein in the PB Outliner; that row going when the assembly is cleared, and never appearing for a deposited assembly (which has no generator settings for its pencil to open on); Apply building the same thing OK would, through one shared path; and Cancel putting back *whichever* of the three states the dialog opened on - nothing, an earlier generated symmetry, or a deposited assembly. Then the two traps: repeated Applies still cancelling back to what the dialog opened on rather than to the previous preview, and OK keeping only the protein it committed while a protein that was merely previewed goes back. Plus the record that makes editing possible - settings living with the build so a second protein's build does not move the first's, dying with `clear_assembly`, absent on a deposited assembly, carrying the Trim limits, and seeding field by field so one stored value a property will not take costs that control rather than the dialog |
